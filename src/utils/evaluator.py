...
#返回字典，key是实体类型（去掉前缀S,B,I,E)，value是文本内容 + 对应index
    ...
    while index_ < len(decode_tokens):
        ...
        if token_label[0].startswith('S'):
            ...
            if token_type not in predict_entities:
                ...
                predict_entities[token_type].append((tmp_ent, int(index_))) #原文、第几个token
