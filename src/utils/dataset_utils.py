...
class NERDataset(Dataset): ##建立torch dataset
    def __init__(self, task_type, features, mode, **kwargs):
        ...
        self.token_ids = [torch.tensor(example.token_ids,dtype = torch.long) for example in features]
        #self.attention_masks = [torch.tensor(example.attention_masks).float() for example in features]
        self.attention_masks = [torch.tensor(example.attention_masks,dtype = torch.float) for example in features]
        #self.token_type_ids = [torch.tensor(example.token_type_ids).long() for example in features]
        self.token_type_ids = [torch.tensor(example.token_type_ids,dtype = torch.long) for example in features]
        ...
        if mode == 'train':
            ...
            #self.pseudo = [torch.tensor(example.pseudo,dtype=torch.long) for example in features]
            if task_type == 'crf':
                ...
                self.start_ids = [torch.tensor(example.start_ids,dtype = torch.long) for example in features]
                #self.end_ids = [torch.tensor(example.end_ids).long() for example in features]
                self.end_ids = [torch.tensor(example.end_ids,dtype = torch.long) for example in features]
